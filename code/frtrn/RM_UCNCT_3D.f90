...
PROGRAM RM_UCNCT_3D
   ...
   !-----------------------------------------------------------------
   ! working grid sizes and frame variables
   ...
   !-----------------------------------------------------------------
   ! domain parameters
   REAL(KIND=8), ALLOCATABLE :: psA(:,:,:), psB(:,:,:)
   ! connectivity field
   REAL(KIND=8), ALLOCATABLE :: Cn(:,:,:)
   ! geometry input
   REAL(KIND=8), ALLOCATABLE, DIMENSION(:,:,:) :: RdIn
   ! sign function and distance function
   ...
   DO N = 3, 13 , 1
      ...
      ! convert distance field to solid domain parameter
      ...
      !! load connectivity for solid particles
      WRITE(frm,"(I3)") 100+fr
      OPEN(UNIT=20,FILE= &
         '../Geom/'// &
      ...
      !-----------------------------------------------------------------
      ! Compute distance function for solid phase
      !-----------------------------------------------------------------
      ...
      ! load connectivity for electrolyte
      psB(1:py,1:px,1:pz) = psA(1:py,1:px,1:pz)
      OPEN(UNIT=20,FILE= &
         '../Geom/'// &
      ...
      !-----------------------------------------------------------------
      ! Compute distance function for electrolyte phase
      !-----------------------------------------------------------------
      dsF(1:py,1:px,1:pz) = psB(1:py,1:px,1:pz)
      ! impose no-gradient boundary conditions
      CALL NoGrdBc3D(py, px, pz, dsF(0:py+1,0:px+1,0:pz+1))

      !! sign function for electrolyte phase
      WHERE ( dsF(0:py+1,0:px+1,0:pz+1) > 0.5 )
         sgF(0:py+1,0:px+1,0:pz+1) = 1.0
      ELSEWHERE
         sgF(0:py+1,0:px+1,0:pz+1) = -1.0
      ...
      ! save electrolyte distance field (unformatted)
      ! OPEN(UNIT=30,FILE= &
      ...
      ! save electrolyte distance field (formatted for post-processing)
      OPEN(UNIT=40,FILE= &
         '/mnt/scratch/hcy/SBMES_2022/2022_1225_A/Geom_T/'// &
